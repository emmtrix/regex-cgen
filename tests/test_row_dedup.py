...
def test_dedup_emits_row_map_when_duplicates_exist() -> None:
    """When duplicate rows exist, dfa_row_map must be properly declared and used."""
    ...
    assert re.search(rf"dfa_row_map\[{n}\]", code), (
        "dfa_row_map not declared with the correct state count"
    )
    # The hot loop must reference the row map for the table lookup
    assert "dfa_transitions[dfa_row_map[state]]" in code, (
        "hot loop does not use dfa_row_map for table indexing"
    )


def test_dedup_reduces_table_rows() -> None:
    """The emitted dfa_transitions must have fewer rows than num_states."""
    dfa = compile_regex("a[bc]+d")
    n = dfa["num_states"]
    code = generate_c_code(dfa)
    # Extract the dimension from the table declaration, e.g. "dfa_transitions[4][256]"
    m = re.search(r"dfa_transitions\[(\d+)\]\[256\]", code)
...
def test_dedup_no_row_map_when_all_rows_unique() -> None:
    """When every DFA state has a unique row, no dfa_row_map should be emitted."""
    ...
    code = generate_c_code(dfa)
    assert "dfa_row_map" not in code
